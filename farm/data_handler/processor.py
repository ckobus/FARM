...
from farm.data_handler.input_features import (
    ...
    sample_to_features_nq_multitask
)
...
class SquadbisProcessor(Processor):
    """ Used to handle the SQuAD dataset _ Multitask setup"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(SquadbisProcessor, self).__init__(
            tokenizer=tokenizer,
            max_seq_len=max_seq_len,
            train_filename=train_filename,
            dev_filename=dev_filename,
    ...
    def _sample_to_features(self, sample) -> dict:
        # TODO, make this function return one set of features per sample
        features = sample_to_features_squad_multitask(sample=sample,
                                            tasks=self.tasks,
                                            tokenizer=self.tokenizer,
                                            max_seq_len=self.max_seq_len)
        return features
...
class NQProcessor(Processor):
    """ Used to handle the NQ dataset _ Multitask setup"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(NQProcessor, self).__init__(
            tokenizer=tokenizer,
            max_seq_len=max_seq_len,
            train_filename=train_filename,
            dev_filename=dev_filename,
    ...
    def _dict_to_samples(self, dictionary: dict, **kwargs) -> [Sample]:
        n_special_tokens = self.tokenizer.num_added_tokens(pair=True)
        samples = create_samples_nq(dictionary=dictionary,
                                       max_query_len=self.max_query_length,
                                       max_seq_len=self.max_seq_len,
                                       doc_stride=self.doc_stride,
                                       n_special_tokens=n_special_tokens)
    ...
    def _sample_to_features(self, sample) -> dict:
        # TODO, make this function return one set of features per sample
        features = sample_to_features_nq_multitask(sample=sample,
                                            tasks=self.tasks,
                                            tokenizer=self.tokenizer,
                                            max_seq_len=self.max_seq_len)
        return features